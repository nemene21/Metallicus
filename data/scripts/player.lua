...
function newPlayer(x,y,stats)
    local inventory = newInventory(42,128,5,3)
    local hotbar = newInventory(42,42,5,1,"hotbarSlot")
    local wearing = newInventory(300,128,0,0)
...
function processPlayer(player)
    ...
    player.collider = moveRect(player.collider, player.vel, {})
...
function playerRunAnimation(player)
    local armYOffset = math.sin(globalTimer * 12) * 4
    ...
    local legLYOffset = math.cos(globalTimer * 12) * 3
    local legRYOffset = math.cos(globalTimer * 12 + 1.57) * 3
...
PLAYER_ANIMATIONS = {
idle=playerIdleAnimation, run=playerRunAnimation
}